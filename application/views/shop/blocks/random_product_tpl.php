...
?>
				<div class="box_top">
					...
					<div class="box" id="tip">
                        <p class="highlight"><a href="<?php echo site_url('/shop/product/index/'.$lang.'/'.$product['nicename']) ?>"><?php echo $product['title']; ?></a></p>
						<p>
                            <a href="<?php echo site_url('/shop/product/index/'.$lang.'/'.$product['nicename']) ?>"><img src="<?php echo base_url().$product['thumb']; ?>" style="width: 90%; float:none; border:none;" alt="" /></a>
                            <?php echo $product['category_text']; ?><br />
                            <?php echo $product['price'].' '.$this->lang->line('main_currency'); ?>
                             <a href="#" onclick="javascript:<?php echo $this->ajax->remote_function(array('update'=>'items','url'=>site_url('shop/cart/cart_add/'.$lang), 'hoverclass' => 'cart-active', 'loading' => "Element.show('indicator')", 'complete' => "Element.hide('indicator')", 'method' => 'post', 'with' => "'id="."product".$product['productID']."_".$product['productID']."'")); ?>; return FALSE;"><span class="cart_put"></span></a>
                        </p>
					</div>
