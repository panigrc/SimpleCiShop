<?php
    ...
    function printChilds($arr, $level=0, $vars=array()) {
        ...
        foreach($arr as $item => $key){
            $this_level .= "                    <li><a href=\"".site_url('catalog/index/'. $lang .'/'. $obj->Category_model->get_category_nicename($item));
        ...
        foreach($arr as $item => $key){
            if(count($parent) > 0 && $parent[0] === $item) {
                foreach($parent as $cur_parent) {
                    $this_level .= "                    <ul>\n";
                    $this_level .= "                    <li><a href=\"".site_url('catalog/index/'. $lang .'/'. $obj->Category_model->get_category_nicename($cur_parent));
                    $this_level .= " \">".$obj->Category_model->get_category_name($cur_parent) . "</a>\n";
                    $this_level .= "                    </li>\n";
                    $this_level .= "                    </ul>\n";
                }
                $this_level .= "                    <li><a href=\"".site_url('catalog/index/'. $lang .'/'. $obj->Category_model->get_category_nicename($current));
